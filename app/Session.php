...
class Session extends Model
{
    ...
    public function doPostSession($request, $user)
    {
        ...
        if(!$check)
        {
        	...
        	Session::insert([
                ...
                /*'active' => 1,*/
                'active' => 0,//updated 2-6-2018 by priya
        		]);
    ...
    public function doDeleteSession($id)
    {
        ...
        $getStatus = $getThisIDValue->active;
        if(!$getThisIDValue)
        {
            $input['error'] = ' Sorry ,Invalid Parameter !!!';
        }
        if($getStatus == 1)
        {
            $input['error'] = ' Sorry ,You cannt delete bcoz it is in Active !!!';
        }
        else
        {
            /* check student exists in same session */
            ...
            if($exists_student)
            {
                $input['error'] = "  Student available in this session so can't delete";
            }
            else
            {
    ...
    public function doExportMasterSession($user)
    {
        ...
        $sessions = Session::where('school_id', $user->school_id)
            ->where('active','1')//updated 2-6-2018
            ->select('id', 'session')->get()->toArray();
