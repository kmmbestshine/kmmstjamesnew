...
class Payment extends Model
{
	...
	public function get_payment_student_by_id($school_id,$session_id,$student_id){
		$result = payment::where('school_id',$school_id)->where('session_id',$session_id)->where('student_id',$student_id)->get();
		if(!$result){
	...
	public function get_payment_by_id_parthiban($fee_id,$stu_id){
		$result = payment::where('fee_id',$fee_id)->where('student_id',$stu_id)->get();
		return $result;
	}
	
	public function get_balance_amount($fee_id){
		$result = payment::select('balance_amount','id','fee_detail')->where('fee_id',$fee_id)->first();
		return $result;
	}
	...
	public function get_detail_payment($payment){
		$result = array();
		$paid_student = DB::table('student')->where('id',$payment->student_id)->first();
		$result['fee'] =  DB::table('fee_structure')->where('id',$payment->fee_id)->first();
		$result['student'] = $paid_student;
		$result['school'] = DB::table('school')->where('id',$payment->school_id)->first();
		$result['payment'] = $payment;
		$result['class'] = $this->student->getStudentClass($paid_student->class_id,$paid_student->session_id);
		return $result;
	}
