...
class PrincipalController extends Controller
{
    protected $user;
            ...
            ->where('session_id',$this->active_session->id) //14-4-2018
            ->count();
    ...
    public function deleteAllMarks()
    {
        ...
        {
          $deleted=  Result::where('subject_id',$value)->delete();
        }
        if($deleted){

                return redirect('/users/result')->with('success','Well Done! All Student Marks are Deleted Succesfully!!!');
        
                    }
           
    }
    public function deleteMarks($id,$section_id)
    {
      
              $whereArray = array('subject_id' =>$id,'section_id'=>$section_id);
               
                $deleted = Result::where($whereArray)->delete();


               if($deleted)
               {
                return redirect('/users/result')->with('success','Well Done! Student Mark is deleted Succesfully!!!');
               } 
